...
printf "Creating $dir directory..."
if [ -d $dir ]; then
	printf "\e[0;91m Directory already exists\e[0m\n\n"
else
	mkdir $dir
	printf "\e[0;92m Done\e[0m\n\n"
fi

# download levels from range
for level in $(seq 1 512)
do
	printf "Archiving ID $level as GMD..."
	if [ -e "$dir/$level" ]; then
		printf "\e[0;91m Level already archived.\e[0m\n"
	else
		check=$(curl "$db/downloadGJLevel.php" -d "levelID=$level&secret=Wmfd2893gb7" 2> /dev/null)

		if [ "$check" = "-1" ]; then
			printf "\e[0;91m Level not found\e[0m\n"
		else
			touch $dir/$level.gmd
